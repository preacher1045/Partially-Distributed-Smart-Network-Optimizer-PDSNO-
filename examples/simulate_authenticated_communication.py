...
def main():
    ...
    logger.info(f"OK - Generated shared secret (32 bytes)")
    ...
    logger.info("OK - Message authenticators initialized")
    ...
    logger.info("OK - Global Controller created with authentication")
    ...
    logger.info("OK - Regional Controller created with authentication")
    ...
    logger.info(f"OK - Message signed (nonce: {signed_message['nonce'][:16]}...)")
    
    # Verify signature at GC
    valid, error = gc_auth.verify_message(signed_message, expected_sender=rc.temp_id)
    
    if valid:
        logger.info("OK - GC verified message signature successfully")
        logger.info("OK - Test 1 PASSED: Valid message accepted")
    else:
        logger.error(f"ERROR - Signature verification failed: {error}")
    ...
    logger.info("OK - Original message signed")
    
    # Tamper with the message
    signed_message2['payload']['data'] = "TAMPERED"
    logger.info("WARNING - Message payload tampered: original -> TAMPERED")
    
    # Try to verify tampered message
    valid2, error2 = gc_auth.verify_message(signed_message2)
    
    if not valid2:
        logger.info(f"OK - GC detected tampering: {error2}")
        logger.info("OK - Test 2 PASSED: Tampered message rejected")
    else:
    ...
    logger.info(f"OK - Message signed (nonce: {signed_message3['nonce'][:16]}...)")
    
    # First verification succeeds
    valid3a, error3a = gc_auth.verify_message(signed_message3.copy())
    
    if valid3a:
        logger.info("OK - First verification succeeded")
    else:
        logger.error(f"ERROR - First verification failed: {error3a}")
    
    # Try to replay the same message
    logger.info("WARNING - Attempting replay attack (same nonce)...")
    valid3b, error3b = gc_auth.verify_message(signed_message3.copy())
    
    if not valid3b and "Replay attack" in error3b:
        logger.info(f"OK - GC rejected replay: {error3b}")
        logger.info("OK - Test 3 PASSED: Replay attack prevented")
    else:
        logger.error("ERROR - Replay attack was not detected!")
        logger.error("ERROR - Test 3 FAILED")
    
    # Summary
    logger.info("\n[8/8] Security features demonstrated...")
    logger.info("-" * 60)
    logger.info("OK - HMAC-SHA256 message signing")
    logger.info("OK - Signature verification")
    logger.info("OK - Tamper detection")
    logger.info("OK - Replay attack prevention")
    logger.info("OK - Timestamp validation (5-minute window)")
    logger.info("OK - Nonce-based replay cache")
