...
class NIBStore:
    ...
    def get_controllers_by_region(self, region: str) -> List[Controller]:
        """Get all controllers in a region"""
        with self._get_connection() as conn:
            cursor = conn.execute(
                "SELECT * FROM controllers WHERE region = ?",
                (region,)
            )
        ...
        with self._get_connection() as conn:
            ...
            if existing:
                cursor = conn.execute(
                    ...
                    UPDATE controllers SET
                        role = ?, region = ?, status = ?, validated_by = ?, validated_at = ?,
                        public_key = ?, certificate = ?, capabilities = ?, metadata = ?,
                        version = version + 1
                    ...
                    (
                        ...
                        json.dumps(controller.capabilities), json.dumps(controller.metadata),
            ...
            conn.execute(
                ...
                INSERT INTO controllers (
                    controller_id, role, region, status, validated_by, validated_at,
                    public_key, certificate, capabilities, metadata, version
                ) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
                """,
                (
                    controller.controller_id, controller.role, controller.region, controller.status,
                    controller.validated_by,
                    controller.validated_at.isoformat() if controller.validated_at else None,
                    controller.public_key, controller.certificate,
                    json.dumps(controller.capabilities), json.dumps(controller.metadata),
